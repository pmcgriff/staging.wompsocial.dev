...
if( ! class_exists( "DVK_Update_Manager", false ) ) {
	...
	class DVK_Update_Manager {
		 ...
		 */
		public function __construct( DVK_Product $product, $license_manager ) {
		...
		private function call_remote_api() {
			...
			if( isset( $response->license_check ) && ! empty( $response->license_check ) && $response->license_check != 'valid' ) {
				...
				$this->error_message = __( "This site has not been activated properly on dannyvankooten.com and thus cannot check for future updates. Please activate your site with a valid license key.", $this->product->get_text_domain() );
				add_action( 'admin_notices', array( $this, 'show_update_error' ) );
			}
